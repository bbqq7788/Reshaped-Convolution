...
class ResNet(nn.Module):
    ...
    def __init__(self, block, blocks_num, num_classes=10, include_top=True):
        ...
        self.reshape1 = nn.Sequential(
            nn.Conv2d(512*16, 512, 3, 1, 1, groups=512, bias=False),
            nn.BatchNorm2d(512),
            nn.ReLU(),
        )
        self.reshape2 = nn.Sequential(
            nn.Conv2d(1024*4, 1024, 3, 1, 1, groups=1024, bias=False),
            nn.BatchNorm2d(1024),
            nn.ReLU(),
        )

        self.last_conv = nn.Sequential(
            nn.Conv2d(in_channels=3584, out_channels=2048, kernel_size=1, stride=1, bias=False),
            nn.BatchNorm2d(2048),
            nn.ReLU(),
        )
    ...
    def forward(self, x):
        ...
        ####################
        B, C, H, W = x.shape
        out = x.view(B, -1, H // 4, W // 4)
        first = self.reshape1(out)
        ####################
        x = self.layer3(x)
        ####################
        B, C, H, W = x.shape
        out = x.view(B, -1, H//2, W//2)
        second = self.reshape2(out)
        ####################
        x = self.layer4(x)
        x_ = torch.cat((first, second, x), dim=1)
        if self.include_top:
            x = self.last_conv(x_)
            x = self.avgpool(x)
            x = torch.flatten(x, 1)
